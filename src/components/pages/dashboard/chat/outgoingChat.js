    ...
    return (
      <div key={this.props.index} className="flex justify-end mb-2">
        <div className="outgoing-chat shadow-sm py-2 px-3">
          ...
          <p className="text-xs t break-words mt-1">{this.props.chat.message}</p>
          <p style={{fontSize:'8px'}} class="text-grey-dark text-right mt-1">{moment(this.props.chat.timestamp.toDate()).format('LT')}</p>
        </div>
      </div>
    );
  }
